...
void parser_Plot(string& sCmd, Datafile& _data, Parser& _parser, Settings& _option, Define& _functions, PlotData& _pData)
{
    ...
    //_pData.prepareGraph();
    mglData** _mDataPlots = 0;              // 2D-Pointer auf ein Array aus mglData-Objekten
    ...
    if (findCommand(sCmd).sString == "plotcompose")
    {
        ...
        {
            if (vPlotCompose[i].find("-set") != string::npos && !isInQuotes(vPlotCompose[i], vPlotCompose[i].find("-set")))
                _pInfo.sPlotParams += vPlotCompose[i].substr(vPlotCompose[i].find("-set"));
            else if (vPlotCompose[i].find("--") != string::npos && !isInQuotes(vPlotCompose[i], vPlotCompose[i].find("--")))
                ...
                && _pInfo.sPlotParams.find('=') != string::npos)
            {
                ...
                while (_pInfo.sPlotParams.find('=', nPos) != string::npos)
                {
                    ...
                    if (_pInfo.sPlotParams[nPos] == '(')
                    {
                        ...
                        {
                            nPos_temp = nPos+1;
                            while (!containsStrings(_pInfo.sPlotParams.substr(nPos, nPos_temp-nPos)) && !_data.containsStringVars(_pInfo.sPlotParams.substr(nPos, nPos_temp-nPos)))
                                nPos_temp++;
                            nPos = nPos_temp-1;
                        }
                    }
                    for (unsigned int i = nPos; i < _pInfo.sPlotParams.length(); i++)
                    {
                        ...
                        if (((_pInfo.sPlotParams[i] == ' ' || _pInfo.sPlotParams[i] == ')') && !isInQuotes(_pInfo.sPlotParams, i)) || i+1 == _pInfo.sPlotParams.length())
                        {
                            ...
                            if (i+1 == _pInfo.sPlotParams.length())
                                ...
                                _pInfo.sPlotParams.replace(nPos, i-nPos, sParsedString);// = _pInfo.sPlotParams.substr(0, nPos) + sParsedString + _pInfo.sPlotParams.substr(i);
                            break;
                        }
                    }
                }
            }
            //_pInfo.sPlotParams = "-set minline=\"\" maxline=\"\" hline=\"\" vline=\"\" lborder=\"\" rborder=\"\" " + _pInfo.sPlotParams;
            //cerr << _pInfo.sPlotParams << endl;
    ...
    for (unsigned int nPlotCompose = 0; nPlotCompose < vPlotCompose.size(); nPlotCompose++)
    {
                    ...
                    for (int l = 0; l < i_pos[1]-i_pos[0]; l++)
                    {
                        ...
                        for (int q = 0; q < 3; q++)
                        {
                            ...
                            if (l == 0 && i == 0)
                            {
                                ...
                                dDataRanges[q][0] = _mDataPlots[i][q].a[l];
                                dDataRanges[q][1] = _mDataPlots[i][q].a[l];
                                //cerr << dDataRanges[q][0] << endl;
                                //cerr << dDataRanges[q][1] << endl;
                            }
                            else
                            {
                                if (_pInfo.b2D && q == 2)
                                ...
                                else
                                {
                                    if (q && _pData.getRangeSetting())
                                    {
                                        //cerr << _pData.getRanges(0,0) << "  " << _mDataPlots[i][0].a[l] << endl;
                                        //cerr << _pData.getRanges(0,1) << "  " << _mDataPlots[i][0].a[l] << endl;
                                        if (_pData.getRanges(0,0) > _mDataPlots[i][0].a[l] || _pData.getRanges(0,1) < _mDataPlots[i][0].a[l])
                                            continue;
                                    }
                                    if (dDataRanges[q][0] > _mDataPlots[i][q].a[l] || isnan(dDataRanges[q][0]))
        ...
        if (parser_ExprNotEmpty(sFunc) && !(_pInfo.bDraw3D || _pInfo.bDraw))
        {
            ...
            if ((_pData.getColorMask() || _pData.getAlphaMask()) && _pInfo.b2D && nFunctions % 2)
            {
                if (_mDataPlots)
                {
                    for (int i = 0; i < nDataPlots; i++)
        ...
        if (!nPlotCompose)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (_option.getbDebug())
                {
                    cerr << "|-> DEBUG: " << _pInfo.dRanges[i][0] << ":" << _pInfo.dRanges[i][1] << endl;
                }
            }
        ...
        for (int t_animate = 0; t_animate <= _pData.getAnimateSamples(); t_animate++)
        {
            ...
            {
                /*if (_pData.getColorbar() && !_pInfo.b3DVect && _pInfo.sCommand != "plot3d")
            ...
            if (_pInfo.nMaxPlotDim > 2 && !nPlotCompose)
            {
                _graph.Rotate(_pData.getRotateAngle(), _pData.getRotateAngle(1));
            }
            ...
            if (!(_pInfo.b3D || _pInfo.b3DVect || _pInfo.b2DVect) && !nPlotCompose && !(_pInfo.bDraw3D || _pInfo.bDraw))
            {
                if (!_pInfo.b2D && _pData.getGivenRanges() < 2 && _pInfo.sCommand != "plot3d")
                {
                    if (sFunc != "<<empty>>")
                    {
                        if (_mDataPlots && _pData.getMin() < _pInfo.dRanges[1][0])
                            _pInfo.dRanges[1][0] = _pData.getMin();
                        if (_mDataPlots && _pData.getMax() > _pInfo.dRanges[1][1])
                            _pInfo.dRanges[1][1] = _pData.getMax();
                        if (!_mDataPlots)
                        {
                            _pInfo.dRanges[1][0] = _pData.getMin();
                            _pInfo.dRanges[1][1] = _pData.getMax();
                ...
                else if (_pInfo.sCommand == "plot3d" && _pData.getGivenRanges() < 3)
                {
                    if (sFunc != "<<empty>>")
                    {
                        ...
                        if (_pData.getCoords() && _pInfo.dRanges[1][0] != 0.0)
                            _pInfo.dRanges[1][0] = 0.0;
                        /*if (_pData.getCoords() && _pInfo.dRanges[1][1] != 2.0)
                            _pInfo.dRanges[1][1] = 1.99999999;
                        if (_pData.getCoords() == 2 && _pInfo.dRanges[2][1] != 1.0)
                            _pInfo.dRanges[2][1] = 0.99999999;*/
            ...
            else if (_pInfo.b2DVect && !nPlotCompose && !(_pInfo.bDraw3D || _pInfo.bDraw))
            {
                if (sFunc != "<<empty>>")
                {
                ...
                if (_pData.getLighting())
                {
                    ...
                    if (!_pData.getPipe() && !_pInfo.bDraw)
                    {
                        if (_pData.getLighting() == 1)
                        {
                            ...
                            //parser_directionalLight(_graph, _pInfo.dRanges, _pData.getRotateAngle(1), _pData.getRotateAngle(0), 3, 'w', 0.025);
                        }
                        else
                        {
                            ...
                            //parser_directionalLight(_graph, _pInfo.dRanges, _pData.getRotateAngle(1), _pData.getRotateAngle(0), 3, 'w', 0.015);
                        }
                        //_graph.AddLight(3, mglPoint(-30,5,1), 'w', 0.5);
                    }
                    else
                    {
                        if (_pData.getLighting() == 1)
            ...
            if (!nPlotCompose && _pInfo.nMaxPlotDim > 2)
            ...
            if (_pInfo.b2D)        // 2D-Plot
            {
                    ...
                    for (int k = 0; k < nFunctions; k++)
                    {
                        ...
                        for (long int i = 0; i < _pInfo.nSamples; i++)
                        {
                            for (long int j = 0; j < _pInfo.nSamples; j++)
                            {
                                ...
                                _mData.a[i+_pInfo.nSamples*j] = _pData.getData(i,j,k);
                                if (_pData.getColorMask() || _pData.getAlphaMask())
                                    _mMaskData.a[i+_pInfo.nSamples*j] = _pData.getData(i,j,k+1);
                            }
                        }
                        if (_option.getbDebug())
                ...
                if (_mDataPlots)
                {
                    ...
                    {
                        if (_pData.getCoords())
                            _mDataPlots[j][0] = parser_fmod(_mDataPlots[j][0], 2.0*M_PI);
                        if (_pData.getCoords() == 2)
                            _mDataPlots[j][1] = parser_fmod(_mDataPlots[j][1], 1.0*M_PI);
                        ...
                        {
                            if (_pData.getBars())
                                _graph.Tile(_mDataPlots[j][0], _mDataPlots[j][1], _mDataPlots[j][2], _pData.getColorScheme().c_str());
                            else
                        ...
                        if (_pData.getContLabels() && _pInfo.sCommand.substr(0,4) != "cont" && nDataPlots == 1)
                        {
                ...
                if (sFunc != "<<empty>>")
                {
                    ...
                    for (int j = 0; j < nFunctions; j++)
                    {
                        ...
                        if (!_pData.getArea() && !_pData.getRegion())
                            _graph.Plot(_mAxisVals[0], _mData, sLineStyles[nStyle].c_str());
                        else if (_pData.getRegion() && j+1 < nFunctions)
                        {
                            mglData _mData2(_pInfo.nSamples);
                            for (long int i = 0; i < _pInfo.nSamples; i++)
                                _mData2.a[i] = _pData.getData(i,j+1);

                            if (nStyle == nStyleMax-1)
                                _graph.Region(_mAxisVals[0], _mData, _mData2, ("{"+_pData.getColors().substr(nStyle,1) + "7}{" + _pData.getColors().substr(0,1)+"7}").c_str());
                            else
                                _graph.Region(_mAxisVals[0], _mData, _mData2, ("{"+_pData.getColors().substr(nStyle,1) + "7}{" + _pData.getColors().substr(nStyle+1,1)+"7}").c_str());
                            _graph.Plot(_mAxisVals[0], _mData, sLineStyles[nStyle].c_str());
                            if (nStyle == nStyleMax-1)
                                _graph.Plot(_mAxisVals[0], _mData2, sLineStyles[0].c_str());
                            else
                                _graph.Plot(_mAxisVals[0], _mData2, sLineStyles[nStyle+1].c_str());
                            for (int k = 0; k < 2; k++)
                            {
                                nPos = sLabels.find(';');
                ...
                if (_mDataPlots)
                {
                    ...
                    else
                    {
                        for (int j = 0; j < nDataPlots; j++)
                        {
                            ...
                            if (_pData.getxError() || _pData.getyError())
                            {
                                for (long int i = 0; i < _mDataPlots[j][0].GetNx(); i++)
                                {
                                    if (_mDataPlots[j][0].a[i] < _pInfo.dRanges[0][0] || _mDataPlots[j][0].a[i] > _pInfo.dRanges[0][1]
                                        || _mDataPlots[j][1].a[i] < _pInfo.dRanges[1][0] || _mDataPlots[j][1].a[i] > _pInfo.dRanges[1][1])
                                    {
                                        _mDataPlots[j][0].a[i] = NAN;
                                        _mDataPlots[j][1].a[i] = NAN;
                                    }
                                }
                            }
                            ...
                            if (!_pData.getxError() && !_pData.getyError())
                            {
                                ...
                                if (_pData.getInterpolate() && _mDataPlots[j][0].GetNx() >= _pInfo.nSamples)
                                {
                                    ...
                                    /*else if (_pData.getRegion() && j+1 < nDataPlots)
                                    {
                                        ...
                                        cerr << j << endl;
                                        cerr << nStyle << endl;
                                        cerr << _mDataPlots[j+1][0].a[0] << endl;
                                        cerr << _mDataPlots[j+1][1].a[0] << endl;
                                        cerr << ("{"+_pData.getColors().substr(nStyle,1) +"7}{"+ _pData.getColors().substr(nStyle+1,1)+"7}") << endl;
                                        if (nStyle == nStyleMax-1)
                                ...
                                else
                                {
                                    if (!_pData.getArea() && !_pData.getBars() && !_pData.getHBars() && !_pData.getStepplot())
                                        ...
                                        _graph.Barh(_mDataPlots[j][1], _mDataPlots[j][0], (sLineStyles[nStyle]+"^").c_str());
                                    else if (!_pData.getBars() && !_pData.getArea() && !_pData.getHBars() && _pData.getStepplot())
                                        _graph.Step(_mDataPlots[j][0], _mDataPlots[j][1], (sLineStyles[nStyle]).c_str());
                                    else
                            ...
                            if (sConvLegends != "\"\"")
                            {
                                ...
                                if (!_pData.getxError() && !_pData.getyError())
                                {
                                    if ((_pData.getInterpolate() && _mDataPlots[j][0].GetNx() >= _pInfo.nSamples) || _pData.getBars())
                                        _graph.AddLegend(fromSystemCodePage(replaceToTeX(sConvLegends.substr(1,sConvLegends.length()-2))).c_str(), parser_getLegendStyle(sLineStyles[nStyle], _pData).c_str());
                                    else if (_pData.getConnectPoints() || (_pData.getInterpolate() && _mDataPlots[j][0].GetNx() >= 0.9 * _pInfo.nSamples))
            ...
            else if (_pInfo.b3D)   // 3D-Plot
            {
                if (sFunc != "<<empty>>")
                {
                    ...
                    {
                        /*mglData x(_pInfo.nSamples), y(_pInfo.nSamples), z(_pInfo.nSamples);
                        for (int n = 0; n < _pInfo.nSamples; n++)
                        {
                            x.a[n] = n*(_pInfo.dRanges[0][1]-_pInfo.dRanges[0][0])/(double)(_pInfo.nSamples-1)+_pInfo.dRanges[0][0];
                            y.a[n] = n*(_pInfo.dRanges[1][1]-_pInfo.dRanges[1][0])/(double)(_pInfo.nSamples-1)+_pInfo.dRanges[1][0];
                            z.a[n] = n*(_pInfo.dRanges[2][1]-_pInfo.dRanges[2][0])/(double)(_pInfo.nSamples-1)+_pInfo.dRanges[2][0];
                        }*/
                        if (_pData.getHighRes() || !_option.getbUseDraftMode())
                        {
                            if (_pData.getContFilled() && _pData.getContProj())
                    ...
                    /*if (_pData.getContLabels() && _pInfo.sCommand.substr(0,4) != "cont" && nFunctions == 1)
                    {
                        _graph.Cont(_mContVec, _mData, ("t"+sContourStyles[nStyle]).c_str());
                        if (_pData.getContFilled())
                            _graph.ContF(_mContVec, _mData);
                    }*/
                }
            ...
            else if (_pInfo.b3DVect)   // 3D-Vektorplot
            {
                if (sFunc != "<<empty>>")
                {
                    ...
                    /*mglData _mContVec(15);
                    double dMin = min(min(_pInfo.dRanges[0][0], _pInfo.dRanges[1][0]), _pInfo.dRanges[2][0]);
                    double dMax = max(max(_pInfo.dRanges[0][1], _pInfo.dRanges[1][1]), _pInfo.dRanges[2][1]);
                    for (int nCont = 0; nCont < 15; nCont++)
                    {
                        _mContVec.a[nCont] = nCont*(dMax - dMin)/14.0+dMin;
                    }
            ...
            else if (_pInfo.b2DVect)   // 2D-Vektorplot
            {
                if (sFunc != "<<empty>>")
                {
                    ...
                    {
                        //_graph.SetCutBox(mglPoint(0,0,-1), mglPoint(1,1,1.1));
                        _graph.SetCutBox(parser_CalcCutBox(_pData.getRotateAngle(1), _pInfo.dRanges), parser_CalcCutBox(_pData.getRotateAngle(1), _pInfo.dRanges, 1));
                    }*/
                    // --> Entsprechend dem gewuenschten Plotting-Style plotten <--
                    if (_pData.getFlow())
            ...
            else if (_pInfo.bDraw)
            {
                ...
                for (unsigned int v = 0; v < vDrawVector.size(); v++)
                {
                    ...
                    else if (vDrawVector[v].substr(0,10) == "trianglev(")
                    {
                        if (nFunctions < 4)
                            continue;
                        double c = sqrt(3.0)/2.0;//hypot(vResults[2],vResults[3])/2.0*sqrt(3)/hypot(vResults[2],vResults[3]);
                        if (nFunctions < 6)
            ...
            else            // 3D-Trajektorie
            {
                ...
                if (sFunc != "<<empty>>")
                {
                    ...
                    {
                        StripSpaces(sLabels);
                        //cerr << "coords" << _pData.getCoords() << endl;
                        for (long int i = 0; i < _pInfo.nSamples; i++)
                        {
                            for (int j = 0; j < 3; j++)
                            {
                ...
                if (_mDataPlots)
                {
                    ...
                    {
                        if (_pData.getCoords())
                            _mDataPlots[j][1] = parser_fmod(_mDataPlots[j][1], 2.0*M_PI);
                        if (_pData.getCoords() == 2)
                            _mDataPlots[j][2] = parser_fmod(_mDataPlots[j][2], 1.0*M_PI);
                        ...
                        if (_pData.getxError() || _pData.getyError())
                        {
                            for (long int i = 0; i < _mDataPlots[j][0].GetNx(); i++)
                            {
                                if (_mDataPlots[j][0].a[i] < _pInfo.dRanges[0][0] || _mDataPlots[j][0].a[i] > _pInfo.dRanges[0][1]
                                    || _mDataPlots[j][1].a[i] < _pInfo.dRanges[1][0] || _mDataPlots[j][1].a[i] > _pInfo.dRanges[1][1]
                                    || _mDataPlots[j][1].a[i] < _pInfo.dRanges[2][0] || _mDataPlots[j][1].a[i] > _pInfo.dRanges[2][1])
                                {
                                    _mDataPlots[j][0].a[i] = NAN;
                                    _mDataPlots[j][1].a[i] = NAN;
                                    _mDataPlots[j][2].a[i] = NAN;
                                }
                            }
                        }
    ...
    if (!_pData.getAnimateSamples() || !bAnimateVar)
    {
        ...
        {
            //moveFile("mglcolors.tex", sOutputName.substr(0,sOutputName.find(".tex"))+"colors.tex");
    ...
    if (_option.getViewerPath().length() && _option.getEditorPath().length() && _pData.getOpenImage() && !_pData.getSilentMode() && _option.getSystemPrintStatus())
    {
        ...
        //cerr << toSystemCodePage("|-> Viewer-Fenster schließen, um fortzufahren ... ");
