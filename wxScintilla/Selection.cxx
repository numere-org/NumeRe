...
SelectionPosition Selection::First() const {
	SelectionPosition firstPosition;
	if (ranges.size())
		firstPosition = ranges[0].caret;
	for (size_t i=0; i<ranges.size(); i++) {
		if (firstPosition > ranges[i].caret)
			firstPosition = ranges[i].caret;
		if (firstPosition > ranges[i].anchor)
			firstPosition = ranges[i].anchor;
	}
