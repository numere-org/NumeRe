...
namespace url
{
    static std::map<long, std::string> HTTP_RESPONSES = {
        {400, "Bad Request"},
        {401, "Unauthorized"},
    ...
    std::string get(const std::string& sUrl, const std::string& sUserName, const std::string& sPassWord, const std::vector<std::string>& httpHeader)
    {
        ...
        long response_code = curl.getHttpResponse();

        if (response_code >= 400 && response_code < 500)
            throw Error(toString((int)response_code) + " " + HTTP_RESPONSES[response_code] + " [Client error] for GET/" + sUrl);

        if (response_code >= 500)
            throw Error(toString((int)response_code) + " " + HTTP_RESPONSES[response_code] + " [Server error] for GET/" + sUrl);
    ...
    size_t put(const std::string& sUrl, const std::string& sFileName, const std::string& sUserName, const std::string& sPassWord, const std::vector<std::string>& httpHeader)
    {
        ...
        if (response_code >= 400 && response_code < 500)
            throw Error(toString((int)response_code) + " " + HTTP_RESPONSES[response_code] + " [Client error] for PUT/" + sUrl);
    ...
    std::string post(const std::string& sUrl, const std::string& sUserName, const std::string& sPassWord, const std::vector<std::string>& httpHeader, const std::string& sPayLoad)
    {
        ...
        if (response_code >= 400 && response_code < 500)
            throw Error(toString((int)response_code) + " " + HTTP_RESPONSES[response_code] + " [Client error] for POST/" + sUrl);

        if (response_code >= 500)
            throw Error(toString((int)response_code) + " " + HTTP_RESPONSES[response_code] + " [Server error] for POST/" + sUrl);
