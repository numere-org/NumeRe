...
///
/////////////////////////////////////////////////
void TabArtProvider::DrawBackground(wxDC& dc, wxWindow* WXUNUSED(wnd), const wxRect& rect)
{
    // draw background
                             ...
                             int close_button_state, wxRect* out_tab_rect, wxRect* out_button_rect, int* x_extent)
{
    ...
    {
        if (focused && wnd->GetParent()->IsDescendant(focused))
            hasFocus = focused == page.window || page.window->IsDescendant(focused) || focused == wnd;
        else // Kind of a Hack and only works for the editor
            hasFocus = static_cast<NumeReApp*>(wxTheApp)->getMainWindow()->GetTitle().substr(0, page.caption.length()+3) == page.caption + " - ";
    }
