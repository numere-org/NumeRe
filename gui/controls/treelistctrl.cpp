...
// this is the "true" control
    class  wxTreeListMainWindow: public wxScrolledWindow
    {
        ...
        public:
            ...
            {
                return m_rootItem;    // implict cast from wxTreeListItem *
            }
            ...
            {
                return m_curColumn >= 0 ? m_curColumn : m_main_column;
            }
...
//                if (abs_p.y >= GetVirtualSize().GetHeight()) abs_p.y = GetVirtualSize().GetHeight() - 1;
            ...
            default:
                if (event.GetKeyCode() >= (int)' ' && event.GetKeyCode() < WXK_START)
                {
...
// prepare edit (position)
        ...
        switch (header_win->GetColumnAlignment (column))
        {
            ...
            case wxALIGN_RIGHT:
                {
                    style = wxTE_RIGHT;     // yes, strange but that's the way it is
                    x += 0;
                    break;
                }
        }
