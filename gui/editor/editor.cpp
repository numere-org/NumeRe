...
NumeReEditor::NumeReEditor(NumeReWindow* mframe, wxWindow* parent, wxWindowID id,
    ...
    wxStyledTextCtrl(parent, id, pos, size, style, name), m_codeParser(*mframe->getTerminal()->getProvider())
{
    m_mainFrame = mframe;
...
bool NumeReEditor::SaveFile( const wxString& filename )
{
    ...
    if (m_codeParser.isDirty())
    {
        g_logger.info("Starting complete re-parse after saving ...");
        parseAll();
    }
...
bool NumeReEditor::Modified()
{
    ...
    return (modified && readonly && canundo) || m_bSetUnsaved;
}
...
void NumeReEditor::OnChar(wxStyledTextEvent& event)
{
    ...
    if (chr == WXK_TAB)
    {
        ...
        int indentWidth = GetIndent();

        SetSelection(newStartPos, newEndPos);

        for (int i = startLine; i <= endline; i++)
        {
            int lineIndent = GetLineIndentation(i);

            if (doIndent)
                SetLineIndentation(i, lineIndent + indentWidth);
            else
                SetLineIndentation(i, lineIndent - indentWidth);
        }
    }
    ...
    // Handle the autocompletion popup
    handleAutoComplete(chr);

    // Handle the comment autowrapping feature
    handleCommentAutoWrap();
...
/////////////////////////////////////////////////
/// \brief Private member function handling the
/// autocompletion popup from OnChar().
///
/// \param chr wxChar
/// \return void
///
/////////////////////////////////////////////////
void NumeReEditor::handleAutoComplete(wxChar chr)
{
    int currentPos = GetCurrentPos();
    ...
    {
        if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC || m_fileType == FILE_NLYT)
        {
            if (GetStyleAt(wordstartpos) == wxSTC_NSCR_PROCEDURES)
            {
                        ...
                        || sSelectedNamespace == "thisfile~")
                {
                    ...
                    sAutoCompList = m_search->FindProceduresInCurrentFile(GetTextRange(wordstartpos, currentPos),
                                    sSelectedNamespace);
                }
                else
                {
                    // If namespace == "main~" (or similiar) then clear it's contents
                    if (sNamespace == "main" || sNamespace == "main~" || sNamespace == "~")
                        sNamespace = "";
...
/////////////////////////////////////////////////
/// \brief Private member function handling the
/// comment autowrap from OnChar(),
///
/// \return void
///
/////////////////////////////////////////////////
void NumeReEditor::handleCommentAutoWrap()
{
    int currentPos = GetCurrentPos();
    int currentLine = GetCurrentLine();
    ...
    if (!isComment || !isNumereFile || !isLineIndicatorActive)
        return;
...
void NumeReEditor::MakeBlockCheck()
{
    ...
    std::string currentWord = GetTextRange(WordStartPosition(GetCurrentPos(), true), WordEndPosition(GetCurrentPos(), true)).ToAscii().data();
...
void NumeReEditor::ShowDwellingCallTip(int charpos)
{
    ...
    {
        // Is it a block?
        int id = getBlockID(selection.ToAscii().data());
        ...
        if (id != wxNOT_FOUND)
        {
            ...
            if (selection != blockDef.startWord)
                nLength = wxToUtf8(sBlock).length();
    ...
    else if (GetStyleAt(charpos) == wxSTC_NSCR_IDENTIFIER
             || GetStyleAt(charpos) == wxSTC_NSCR_CLUSTER
             || GetStyleAt(charpos) == wxSTC_NSCR_CUSTOM_FUNCTION)
    {
        int lineNum = LineFromPosition(charpos);
        std::string sSymbol = wxToUtf8(selection);

        if (!m_codeParser.isValidLine(lineNum))
            parse(lineNum);

        if (!m_codeParser.isSymbol(sSymbol, lineNum, true))
        {
            AdvCallTipCancel();
            return;
        }

        _cTip = m_codeParser.getSymbol(sSymbol, lineNum, true).print();
    }
    ...
    if (_cTip.sDefinition.length())
    {
        ...
        AdvCallTipShow(startPosition, wxFromUtf8(_cTip.sDefinition)
                       + (_cTip.sDocumentation.length() ? "\n" + wxFromUtf8(_cTip.sDocumentation) : ""));
...
void NumeReEditor::OnAutoCompletion(wxStyledTextEvent& event)
{
    ...
    else if (m_options->getSetting(SETTING_B_BLOCKAUTOCOMP).active() && isBlockStart(event.GetText().ToAscii().data(), true) != wxNOT_FOUND)
    {
        // Get the autocompletion block and find
        // the pipe position
        wxString sAutoComp = getBlockAutoCompletion(event.GetText().ToAscii().data());
...
{
    if ((m_fileType != FILE_NSCR && m_fileType != FILE_NPRC)
            || (GetStyleAt(pos) != wxSTC_NPRC_IDENTIFIER && GetStyleAt(pos) != wxSTC_NSCR_CUSTOM_FUNCTION))
        return;

    int lineNum = LineFromPosition(pos);
    ...
    if (!m_codeParser.isSymbol(sSymbol, lineNum, false))
        return;

    const ParserSymbol& symbol = m_codeParser.getSymbol(sSymbol, lineNum, false);
    ParserSymbol currentType(sSymbol);

    if (symbol.getHeuristicEquivalent() == currentType.getHeuristicEquivalent() || symbol.isFunction() || symbol.m_type == "table")
        return;
    ...
    CodeAnalyzer::changeVariableType(sSymbol, sType.front());
    ...
    if (sSymbol != sOldName)
    {
        ...
        // Find all occurences
        std::vector<int> vMatches = m_search->FindAll(sOldName, {wxSTC_NSCR_IDENTIFIER, wxSTC_NSCR_CUSTOM_FUNCTION},
        ...
        if (m_codeParser.isValidLine(LineFromPosition(vMatches.front())))
            m_codeParser.markDirty(LineFromPosition(vMatches.front()));
...
void NumeReEditor::ToggleSettings(int _setting)
{
    ...
    // Re-run the analyzer if it was toggled with this function call
    if (_setting & SETTING_USEANALYZER)
        m_analyzer->run();
}
...
vector<int> NumeReEditor::BlockMatchNSCR(int nPos)
{
    ...
    startblock = GetTextRange(WordStartPosition(nPos, true), WordEndPosition(nPos, true)).ToAscii().data();

    int blockMiddle = isBlockMiddle(startblock);

    if (startblock.starts_with("end"))
    {
    ...
    else if (blockMiddle != wxNOT_FOUND)
    {
        ...
        for (int i = WordEndPosition(nPos, true); i >= 0; i--)
        {
            ...
            {
                std::string currentWord = GetTextRange(WordStartPosition(i, true), WordEndPosition(i, true)).ToAscii().data();
...
void NumeReEditor::UpdateSyntaxHighlighting(bool forceUpdate)
{
    ...
    else if (filetype == FILE_JSON)
    {
        SetLexer(wxSTC_LEX_CPP);
        SetProperty("fold", "1");
        SetKeyWords(0, "true false");
        StyleSetForeground(wxSTC_C_DEFAULT, wxColour(0, 0, 0));
        StyleSetForeground(wxSTC_C_IDENTIFIER, wxColour(0, 0, 0));
        StyleSetForeground(wxSTC_C_NUMBER, wxColour(128, 0, 128));
        StyleSetForeground(wxSTC_C_STRING, wxColour(128, 128, 255));
...
{
    CallTipSetForegroundHighlight(*wxBLUE);
    SetCaretLineVisible(true);
    SetIndentationGuides(wxSTC_IV_LOOKBOTH);

    StyleSetBackground(wxSTC_STYLE_INDENTGUIDE, m_options->GetSyntaxStyle(Options::STANDARD).foreground);
    StyleSetForeground(wxSTC_STYLE_INDENTGUIDE, m_options->GetSyntaxStyle(Options::STANDARD).background);

    SetCaretForeground(m_options->GetSyntaxStyle(Options::STANDARD).foreground);
...
//	this->SetFoldMarginHiColour(true, m_options->GetSyntaxStyle(Options::STANDARD).foreground);
    ...
    if (!m_options->GetSyntaxStyle(Options::ACTIVE_LINE).defaultbackground)
        SetCaretLineBackground(m_options->GetSyntaxStyle(Options::ACTIVE_LINE).background);
    else
    ...
    StyleSetForeground(wxSTC_STYLE_BRACELIGHT, wxColour(0, 150, 0));
    StyleSetBackground(wxSTC_STYLE_BRACELIGHT, wxColour(0, 220, 0));
    StyleSetBold(wxSTC_STYLE_BRACELIGHT, true);
    StyleSetSize(wxSTC_STYLE_BRACELIGHT, this->StyleGetSize(0) + 1);
    StyleSetForeground(wxSTC_STYLE_BRACEBAD, wxColour(150, 0, 0));
    StyleSetBackground(wxSTC_STYLE_BRACEBAD, wxColour(220, 0, 0));
    StyleSetBold(wxSTC_STYLE_BRACEBAD, true);
    StyleSetSize(wxSTC_STYLE_BRACEBAD, this->StyleGetSize(0) + 1);
...
//    StyleSetBackground(ANNOTATION_NOTE, wxColour(240, 240, 240));
//    StyleSetForeground(ANNOTATION_NOTE, wxColour(120, 120, 120));
    StyleSetBackground(ANNOTATION_NOTE, wxColour(222, 237, 255));
    StyleSetForeground(ANNOTATION_NOTE, wxColour(47, 140, 255));
    StyleSetSize(ANNOTATION_NOTE, nAnnotationFontSize);
    StyleSetItalic(ANNOTATION_NOTE, true);
    StyleSetFaceName(ANNOTATION_NOTE, "Segoe UI");

    StyleSetBackground(ANNOTATION_WARN, wxColour(255, 240, 210));
    StyleSetForeground(ANNOTATION_WARN, wxColour(200, 120, 0));
    StyleSetSize(ANNOTATION_WARN, nAnnotationFontSize);
    StyleSetItalic(ANNOTATION_WARN, true);
    StyleSetFaceName(ANNOTATION_WARN, "Segoe UI");

    StyleSetBackground(ANNOTATION_ERROR, wxColour(255, 225, 225));
    StyleSetForeground(ANNOTATION_ERROR, wxColour(170, 0, 0));
    StyleSetSize(ANNOTATION_ERROR, nAnnotationFontSize);
    StyleSetItalic(ANNOTATION_ERROR, true);
    StyleSetFaceName(ANNOTATION_ERROR, "Segoe UI");
}
...
void NumeReEditor::OnRightClick(wxMouseEvent& event)
{
    ...
    if (clickedWord.length() > 0 || HasSelection())
    {
        ...
        else if (isNumeReFileType() && Includer::is_including_syntax(wxToUtf8(GetLine(linenum))))
        {
...
void NumeReEditor::markModified(int nLine)
{
    ...
    while (MarkerGet(nLine) & nMarkSaved)
    {
        MarkerDelete(nLine, MARKER_SAVED);
    }

    if (!(MarkerGet(nLine) & nMarkModified))
        MarkerAdd(nLine, MARKER_MODIFIED);

    m_codeParser.markDirty(nLine);
}
...
void NumeReEditor::markSaved()
{
    ...
    while ((nNextLine = MarkerNext(0, nMarkModified)) != -1)
    {
        MarkerDelete(nNextLine, MARKER_MODIFIED);

        if (!(MarkerGet(nNextLine) & nMarkSaved))
            MarkerAdd(nNextLine, MARKER_SAVED);
    }
...
void NumeReEditor::OnEditorModified(wxStyledTextEvent& event)
{
    if (!m_bLoadingFile && (event.GetModificationType() & wxSTC_MOD_INSERTTEXT || event.GetModificationType() & wxSTC_MOD_DELETETEXT))
    {
        ...
        bool numereFile = isNumeReFileType();
        //g_logger.info("Line " + toString(nLine+1) + ": " + toString(nLinesAdded) + " added");
        ...
        if (nLinesAdded > 0)
        {
            if (numereFile)
            {
                ...
                {
                    m_codeParser.insertLine(nLine);
                }
            }

            for (int i = 0; i < nLinesAdded; i++)
            {
                markModified(nLine + nLinesAdded - 1 - i);
            }
        ...
        else if (nLinesAdded < 0)
        {
            ...
            if (numereFile)
            {
                for (int i = 0; i < -nLinesAdded; i++)
                {
                    m_codeParser.deleteLine(nLine + 1);
                }
            }
...
void NumeReEditor::AsynchEvaluations()
{
    ...
    {
        if (isNumeReFileType() && !m_codeParser.isValid())
            parseAll();
...
void NumeReEditor::RenameSymbols(int nPos)
{
    ...
    if ((m_fileType == FILE_NPRC || m_fileType == FILE_MATLAB) && !textdialog.replaceInWholeFile())
    {
        ...
        // the located head
        std::vector<int> vBlock = BlockMatch(nStartPos);
    ...
    int nStyle = GetStyleAt(nPos);

    // Ensure that the new symbol is not already in use
    std::vector<int> vNewNameOccurences = m_search->FindAll(sNewName, nStyle, nStartPos, nEndPos);
    ...
    if (!vNewNameOccurences.size() && (nStyle == wxSTC_NSCR_IDENTIFIER || nStyle == wxSTC_NSCR_CUSTOM_FUNCTION))
        vNewNameOccurences = m_search->FindAll(sNewName, {wxSTC_NSCR_IDENTIFIER, wxSTC_NSCR_CUSTOM_FUNCTION}, nStartPos, nEndPos);
    ...
    if (vNewNameOccurences.size())
    {
        ...
        if (wxMessageBox(_guilang.get("GUI_DLG_RENAMESYMBOLS_ALREADYINUSE_WARNING"),
                         _guilang.get("GUI_DLG_RENAMESYMBOLS_ALREADYINUSE"),
                         wxCENTER | wxOK | wxCANCEL | wxICON_EXCLAMATION, this) == wxCANCEL)
            return;
    }

    // Gather all operations into one undo step
    std::vector<int> vOccurences;

    if (m_codeParser.isValid() && m_codeParser.isSymbol(wxToUtf8(sCurrentName), LineFromPosition(nPos), false))
    {
        const ParserSymbol& symbol = m_codeParser.getSymbol(wxToUtf8(sCurrentName), LineFromPosition(nPos), false);

        if (symbol.m_type == "value" || symbol.m_type == "{value}")
            vOccurences = m_search->FindAll(sCurrentName,
        {wxSTC_NSCR_IDENTIFIER, wxSTC_NSCR_CUSTOM_FUNCTION},
        nStartPos, nEndPos, textdialog.replaceInComments());
        else
            vOccurences = m_search->FindAll(sCurrentName, nStyle, nStartPos, nEndPos, textdialog.replaceInComments());
    }
    else
        vOccurences = m_search->FindAll(sCurrentName, nStyle, nStartPos, nEndPos, textdialog.replaceInComments());
    ...
    ReplaceMatches(vOccurences, sCurrentName, sNewName);

    if (m_codeParser.isValidLine(LineFromPosition(vOccurences.front())))
        m_codeParser.markDirty(LineFromPosition(vOccurences.front()));
...
/// \param vMatch const std::vector<int>&
/// \return bool
///
...
/////////////////////////////////////////////////
bool NumeReEditor::IsModifiedInSection(int nSectionStart, int nSectionEnd, const wxString& sToken, const std::vector<int>& vMatch)
{
...
wxString NumeReEditor::generateAutoCompList(int wordstartpos, int currpos, std::string sPreDefList)
{
    ...
    int lineNum = LineFromPosition(currpos);
    ...
    if (isNumeReFileType())
    {
        ...
        {
            std::vector<ParserSymbol> candidates = m_codeParser.getCandidates(wxToUtf8(wordstart), lineNum, true);

            for (const ParserSymbol& symbol : candidates)
            {
                if (symbol.m_type == "table")
                    sScopedList += symbol.m_symbol + "(?" + toString((int)NumeReSyntax::SYNTAX_TABLE) + " ";
                else if (symbol.m_type == "cluster")
                    sScopedList += symbol.m_symbol + "{?" + toString((int)NumeReSyntax::SYNTAX_CLUSTER) + " ";
                else if (symbol.isFunction())
                    sScopedList += symbol.m_symbol + "(?" + toString((int)NumeReSyntax::SYNTAX_FUNCTION) + " ";
                else
                    sScopedList += symbol.m_symbol + "?" + toString((int)NumeReSyntax::SYNTAX_STD) + " ";
            }
    ...
    if (useSmartSense)
        context = getCurrentContext(lineNum);
...
{
    if (!isNumeReFileType() || !Includer::is_including_syntax(wxToUtf8(GetLine(GetCurrentLine()))))
        return;
...
{
    RenameSymbols(this->PositionFromPoint(m_lastRightClick));
}
...
{
    fixSymbolName(PositionFromPoint(m_lastRightClick));
}
...
static bool isEqualIgnoreWhitespace(StringView word1, StringView word2)
{
    word1.strip();
    word2.strip();
...
///
/// \param sWord const std::string&
/// \param allowIntermediate bool
/// \return int
///
/////////////////////////////////////////////////
int NumeReEditor::isBlockStart(const std::string& sWord, bool allowIntermediate)
{
    if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC)
    {
        ...
        if (id != wxNOT_FOUND)
        {
            if (isEqualIgnoreWhitespace(vBlockDefs[id].startWord, sWord)
                    || (allowIntermediate && (isEqualIgnoreWhitespace(sWord, vBlockDefs[id].middleWord1)
                                              || isEqualIgnoreWhitespace(sWord, vBlockDefs[id].middleWord2))))
                return id;
        }
    }
    else if (m_fileType == FILE_MATLAB)
...
///
/////////////////////////////////////////////////
int NumeReEditor::isBlockEnd(const std::string& sWord)
{
    if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC)
    {
        int id = getBlockID(sWord);
...
///
/////////////////////////////////////////////////
int NumeReEditor::isBlockMiddle(const std::string& sWord)
{
    if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC)
    {
        int id = getBlockID(sWord);
        ...
        if (id != wxNOT_FOUND
                && (isEqualIgnoreWhitespace(vBlockDefs[id].middleWord1, sWord)
                    || isEqualIgnoreWhitespace(vBlockDefs[id].middleWord2, sWord)))
            return id;
    }
    else if (m_fileType == FILE_MATLAB)
...
///
/////////////////////////////////////////////////
bool NumeReEditor::hasBlockMiddle(const std::string& sWord)
{
    if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC)
    {
        int id = getBlockID(sWord);
...
///
/// \param word const std::string&
/// \return int
///
/////////////////////////////////////////////////
int NumeReEditor::getBlockID(const std::string& word)
{
    if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC)
    {
        if (!word.length())
...
///
/////////////////////////////////////////////////
std::string NumeReEditor::getBlockAutoCompletion(const std::string& sWord)
{
    if (m_fileType == FILE_NSCR || m_fileType == FILE_NPRC)
    {
...
///
/////////////////////////////////////////////////
std::string NumeReEditor::getBlockEnd(const std::string& sWord)
{
    if (m_fileType == FILE_MATLAB)
        return "end";
...
///
/////////////////////////////////////////////////
std::string NumeReEditor::getBlockStart(const std::string& sWord)
{
    if (m_fileType == FILE_MATLAB)
        return "";
...
///
/////////////////////////////////////////////////
std::pair<std::string, int> NumeReEditor::getBlockStart(int line)
{
    if ((GetFoldLevel(line) & wxSTC_FOLDLEVELNUMBERMASK) == wxSTC_FOLDLEVELBASE)
        return std::make_pair(std::string(), wxNOT_FOUND);
    ...
    for (int n = PositionFromLine(foldstart); n < GetLineEndPosition(foldstart); n++)
    {
        ...
        {
            std::string command = GetTextRange(WordStartPosition(n, true), WordEndPosition(n, true)).ToAscii().data();
...
std::pair<std::string, bool> NumeReEditor::get_method_root_type(int pos)
{
    ...
    // Jump over square brackets
    if (style == wxSTC_NSCR_OPERATORS && c == ']' && BraceMatch(pos - 2) != wxNOT_FOUND)
    {
        pos = BraceMatch(pos - 2) + 1;
        style = GetStyleAt(pos - 2);
        c = GetCharAt(pos - 2);
        lineNum = LineFromPosition(pos);
    }
    ...
    if (style == wxSTC_NSCR_IDENTIFIER)
    {
        ...
        std::string symbol = wxToUtf8(GetTextRange(c_pos, WordEndPosition(c_pos, true)));

        if (m_codeParser.isSymbol(symbol, lineNum, false))
            std::tie(varType, isVect) = CodeParser::convertToMethodType(m_codeParser.getSymbol(symbol, lineNum, false).m_type);
        else
        {
    ...
    else if (c == ')')
    {
        ...
        else if (prevStyle == wxSTC_NSCR_PROCEDURES)
        {
            ...
            varType = "string";
        else if (prevStyle == wxSTC_NSCR_CUSTOM_FUNCTION)
        {
            int symbolStart = WordStartPosition(braceStart - 1, true);
            std::string sSymbolName = wxToUtf8(GetTextRange(symbolStart, braceStart));

            if (m_codeParser.isSymbol(sSymbolName, lineNum, false))
            {
                std::tie(varType, isVect) = CodeParser::convertToMethodType(m_codeParser.getSymbol(sSymbolName,
                                            lineNum, false).m_type);
            }
            else
                varType = "table";
        }
        else if (prevStyle == wxSTC_NSCR_PREDEFS)
        {
...
/////////////////////////////////////////////////
/// \brief Perform a complete parsing step on the
/// current file.
///
/// \return void
///
/////////////////////////////////////////////////
void NumeReEditor::parseAll()
{
    m_codeParser.clear();
    ...
    m_codeParser.setFileName(wxToUtf8(GetFileNameAndPath()));
    m_codeParser.setFileLineCount(GetLineCount());

    // Set all local scopes and procedures
    if (m_fileType == FILE_NPRC)
    {
        std::vector<int> scopeStarts = m_search->FindAll("procedure", wxSTC_NPRC_COMMAND, 0, GetLastPosition(), false);

        for (int scopeStart : scopeStarts)
        {
            std::pair<int, int> ctx = getCurrentContext(LineFromPosition(scopeStart));
            m_codeParser.addLocalScope(ParserScope(ctx.first, ctx.second));
        }

        std::map<std::string, std::string> localProcs;

        for (wxString procdef : m_search->getProceduresInFile())
        {
            wxString returns;
            ...
            if (procdef.find("::") != std::string::npos)
            {
                wxString flags = procdef.substr(procdef.find("::") + 2);
            ...
            if (procdef.length())
                localProcs["$thisfile~" + wxToUtf8(procdef.substr(1, procdef.find('(') - 1))] = wxToUtf8(returns);
        }


        m_codeParser.setLocalProcedures(localProcs);
    }
...
/////////////////////////////////////////////////
/// \brief Run only the necessary parsing steps.
///
/// \param requestFromLine int
/// \return void
///
/////////////////////////////////////////////////
void NumeReEditor::parse(int requestFromLine)
{
    // If nothing has been parsed yet, parse everything
    if (!m_codeParser.isValid())
    {
        g_logger.info("Starting initial parse ...");
        parseAll();

        return;
    }

    // Check, whether the user is currently in a
    // modified section
    ...
    // In all other cases: check first, whether a full reparse is necessary bc.
    // too many or too early changes have been aggregated over time
    if (m_codeParser.needsFullReparse())
    {
        g_logger.info("Starting full reparse ...");
        parseAll();

        return;
    }

    // Only do this, if only single scopes need reparsing
    while (m_codeParser.isDirty() && m_codeParser.isValid())
    {
        const ParserScope& dirtyScope = m_codeParser.getNextDirtyScope();
        g_logger.info("Starting single scope reparse ...");
        parseSection(dirtyScope.m_start, dirtyScope.m_end);
    }
}


/////////////////////////////////////////////////
/// \brief Perform the parsing step on a section
/// of code lines.
///
/// \param startLine int
/// \param endLine int
...
///
/////////////////////////////////////////////////
void NumeReEditor::parseSection(int startLine, int endLine)
{
    // Insert every line and parse
    for (int i = startLine; i < endLine; i++)
    {
                ...
                && i + 1 < GetLineCount())
        {
            lexedTokens.pop_back();
            i++;
            std::vector<LexedString> nextTokens = getLexedTokens(i);
            lexedTokens.insert(lexedTokens.end(), nextTokens.begin(), nextTokens.end());
        }

        m_codeParser.parse(rootLine, i - rootLine, lexedTokens);
    }
...
wxString NumeReEditor::ExtractAsHTML(int nFirstLine, int nLastLine)
{
    ...
    wxTextAttr textAttr;
    auto convertToHtml = [&](wxString textRange, int style)
    {
        wxString sHtml;
        sHtml += "<span style=\"color: rgb("
        ...
        return sHtml + "\">" + textRange + "</span>";
    };

    for (int i = nFirstPos; i < nLastPos; i++)
    {
        if (GetStyleAt(i) != GetStyleAt(nLastStatePosition))
        {
            int style = GetStyleAt(nLastStatePosition);
            wxString textRange = GetTextRange(nLastStatePosition, i);
            ...
            sHtml += convertToHtml(textRange, style);
            nLastStatePosition = i;
        }
        ...
        {
            int style = GetStyleAt(nLastStatePosition);
            wxString textRange = GetTextRange(nLastStatePosition, i + 1);
...
///
/// \param textRange StringView
/// \param lexedTokens std::vector<LexedString>&
/// \return void
///
/////////////////////////////////////////////////
static void splitLexedOperators(StringView textRange, std::vector<LexedString>& lexedTokens)
{
    ...
    while (textRange.length())
    {
        ...
        for (const std::string& op : validMultiByteOperators)
        {
            if (textRange.starts_with(op))
            {
                lexedTokens.push_back(LexedString(op, wxSTC_NSCR_OPERATORS));
                textRange.trim_front(op.length());
                cont = true;
                break;
            }
        }
        ...
        lexedTokens.push_back(LexedString(textRange.subview(0, 1).to_string(), wxSTC_NSCR_OPERATORS));
        textRange.trim_front(1);
    }
}


/////////////////////////////////////////////////
/// \brief Return the selected line separated
/// into lexed tokens.
///
/// \param lineNum int
...
///
/////////////////////////////////////////////////
std::vector<LexedString> NumeReEditor::getLexedTokens(int lineNum)
{
    std::vector<LexedString> lexedTokens;

    int nFirstPos = PositionFromLine(lineNum);
    int nLastPos = GetLineEndPosition(lineNum);
    int nLastStatePosition = nFirstPos;
    ...
    for (int i = nFirstPos; i < nLastPos; i++)
    {
        ...
        {
            int style = GetStyleAt(nLastStatePosition);
            std::string textRange = wxToUtf8(GetTextRange(nLastStatePosition, i));

            StripSpaces(textRange);

            if (textRange.find_first_not_of(" \r\t\n") == std::string::npos || textRange == "|>")
            {
                nLastStatePosition = i;
                continue;
            }

            if (style == wxSTC_NSCR_OPERATORS && textRange.length() > 1)
                splitLexedOperators(textRange, lexedTokens);
            else
    ...
    if (nLastStatePosition < nLastPos)
    {
        int style = GetStyleAt(nLastStatePosition);
        std::string textRange = wxToUtf8(GetTextRange(nLastStatePosition, nLastPos));
