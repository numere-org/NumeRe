...
void Datafile::openCSV(Settings& _option)
{
	if (!bValidData)			// Es sind hoffentlich noch keine Daten gespeichert ...
	{
		...
		/*
		 * --> Jetzt wissen wir, wie groß dDatafile sein muss und koennen den Speicher allozieren <--
		 */
		if (nLines && nCols)
		{
			if (!dDatafile)
	...
	if (!bValidData)			// Es sind hoffentlich noch keine Daten gespeichert ...
	{
		...
		while (!file_in.eof())		// Zaehlen wir mal die Zeilen der Datei
		{
            ...
            if (s.find("##JCAMPDX=") != string::npos)
            {
                ...
                {
                    cerr << LineBreak("|-> HINWEIS: Die Versionsnummer dieses JCAMP-DX-Spektrums ist größer als 5.1. Es besteht die Möglichkeit, dass NumeRe diese Datei nicht korrekt lesen kann. Es wird trotzdem versucht, die Daten zu interpretieren.", _option) << endl;
                }
            }
			// --> Erhoehe den Counter <--
			if (s.length())
