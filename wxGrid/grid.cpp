...
void wxGridCellAttrData::RemoveAttrsFromRow(int row)
{
	size_t count = m_attrs.GetCount();
...
void wxGridCellAttrData::RemoveAttrsFromCol(int col)
{
    size_t count = m_attrs.GetCount();
...
void wxGridCellAttrProvider::SetRowAttr(wxGridCellAttr *attr, int row, bool clear)
{
    if ( !m_data )
        InitData();
	...
	m_data->m_cellAttrs.RemoveAttrsFromRow(row);
}

void wxGridCellAttrProvider::SetColAttr(wxGridCellAttr *attr, int col, bool clear)
{
    if ( !m_data )
        InitData();
	...
	m_data->m_cellAttrs.RemoveAttrsFromCol(col);
}
...
void wxGridTableBase::SetRowAttr(wxGridCellAttr *attr, int row, bool clear)
{
    if ( m_attrProvider )
    {
        attr->SetKind(wxGridCellAttr::Row);
        m_attrProvider->SetRowAttr(attr, row, clear);
    }
    else
    {
        // as we take ownership of the pointer and don't store it, we must
...
void wxGridTableBase::SetColAttr(wxGridCellAttr *attr, int col, bool clear)
{
    if ( m_attrProvider )
    {
        attr->SetKind(wxGridCellAttr::Col);
        m_attrProvider->SetColAttr(attr, col, clear);
    }
    else
    {
        // as we take ownership of the pointer and don't store it, we must
...
void wxGrid::SetRowAttr(int row, wxGridCellAttr *attr, bool clear)
{
    if ( CanHaveAttributes() )
    {
        m_table->SetRowAttr(attr, row, clear);
        ClearAttrCache();
    }
    else
    {
...
void wxGrid::SetColAttr(int col, wxGridCellAttr *attr, bool clear)
{
    if ( CanHaveAttributes() )
    {
        m_table->SetColAttr(attr, col, clear);
        ClearAttrCache();
    }
    else
    {
