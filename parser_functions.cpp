...
void parser_ListFunc(const Settings& _option, const string& sType)
{
    ...
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   ascii(STRING)          VAL    - Gibt die ASCII-Zeichenwerte der Zeichen in STRING zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "trigonometric")
    ...
    if (sType == "all" || sType == "num" || sType == "physics")
        cerr << LineBreak("|   bessel(n,x)            VAL    - Besselfunktion erster Gattung der Ordnung n am Ort x. Definiert für ganz R", _option, true, 0, 36) << endl;
    if (sType == "all" || sType == "physics")
        cerr << LineBreak("|   betheweizsaecker(N,Z)  VAL    - Kernbindungsenergie eines Nuklids (N,Z) in MeV aus der Bethe-Weizsäcker-Massenformel. Definiert für N, Z > 0", _option, true, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "physics")
        cerr << LineBreak("|   Bi(x)                  VAL    - Airyfunktion Bi(x). Definiert auf ganz R", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num")
        cerr << LineBreak("|   binom(n,k)             VAL    - Binomialkoeffzient von n und k. Definiert für natürliche Zahlen >= 0", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   char(STRING,POS)       STR    - Gibt das Zeichen aus der Zeichenkette STRING an der Stelle POS zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "stats")
        cerr << LineBreak("|   cmp(x,y,z,...,CMP,T)   VAL    - Vergleicht x,y,z,... mit CMP und gibt ein Ergebnis entsprechend T = {-2,-1,0,1,2} zurück: T = 0 gibt den Index des Elements zurück, das gleich CMP ist. T = -1 gibt den des größten Elements, das kleiner oder gleich CMP ist, und T = 1 den des kleinsten Elements, das größer oder gleich CMP ist, zurück. Für T = -2 bzw. T = 2 werden die entsprechenden Werte der Elemente zurückgegeben. Wird kein passendes Element gefunden, gibt die Funktion \"nan\" zurück. Definiert auf ganz R", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "stats")
        cerr << LineBreak("|   cnt(x,y,z,...)         VAL    - Gibt die Zahl aller Argumente zurück (auch der ungültigen). Kann auch auf Zeichenketten angewendet werden; gibt in diesem Fall aber mindestens 1 zurück.", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "trigonometric")
    ...
    if (sType == "all" || sType == "num")
        cerr << LineBreak("|   dblfacul(n)            VAL    - Doppelfakultät der natürlichen Zahl n [n!! = n*(n-2)*(n-4)*...]. Definiert für natürliche Zahlen >= 0", _option, false, 0, 36) << endl;
    ...
    if (sType == "all" || sType == "num" || sType == "distrib")
        cerr << LineBreak("|   erf(x)                 VAL    - Wert der gauß'schen Fehlerfunktion an der Stelle x. Definiert auf ganz R", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "distrib")
        cerr << LineBreak("|   erfc(x)                VAL    - Wert der komplementären gauß'schen Fehlerfunktion (erfc(x) := 1-erf(x)) an der Stelle x. Definiert auf ganz R", _option, false, 0, 36) << endl;
    ...
    if (sType == "all" || sType == "num")
        cerr << LineBreak("|   faculty(n)             VAL    - Fakultät der natürlichen Zahl n [n! = n*(n-1)*(n-2)*...*1]. Definiert für natürliche Zahlen >= 0", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string" || sType == "logic")
        cerr << LineBreak("|   findfile(STRING)       LOG    - Gibt true zurück, wenn die Datei STRING existiert, sonst false. Die Datei muss in Form einer Zeichenkette angegeben werden", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   findparam(PAR,STR,C)   VAL    - Gibt die Position zurück, an der Parameter PAR in der Zeichenkette STR enthalten ist, und 0, falls er nicht gefunden werden kann. Falls PAR von einem speziellen Zeichen gefolgt werden soll (z.B. '='), kann dies für C angegeben werden, anderenfalls kann C weggelassen werden. Wenn C angegeben wird, wird die Position um die Anzahl an Leerzeichen zwischen PAR und C nach rechts verschoben zurückgegeben.", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num")
        cerr << LineBreak("|   floor(x)               VAL    - Rundet x stets auf die nächsttiefere Ganzzahl ab", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "distrib")
        cerr << LineBreak("|   gamma(x)               VAL    - Wert der Gamma-Funktion an der Stelle x. Definiert auf R \\ {-n, 0}, wobei n eine natürliche Zahl ist", _option, false, 0, 36) << endl;
    ...
    if (sType == "all" || sType == "num")
        cerr << LineBreak("|   gcd(n,k)               VAL    - Berechnet den größten gemeinsamen Teiler der Ganzzahlen n und k", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   getfilelist(STRING)    STR    - Gibt eine Liste aller Dateien, auf die das Namensschema STRING zutrifft, als Zeichenketten zurück. Es wird nur der Name inkl. Endung und nicht der gesamte Pfad zurückgegeben", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   getindices(STRING,T)   VAL    - Gibt die Indices eines in Form einer Zeichenkette übergebenen Datenobjektzugriffs in Form I1,I2,J1,J2 zurück. T ist ein optionaler Parameter und bestimmt, ob eine Matrix (T=0), Spalten- (T=1) oder Zeilenweise (T=2) gelesen werden soll. Mit T=-1 werden unmodifizerte Indices zurückgegeben: 0 für nicht-existente und -1 für offene Intervallgrenzen. Bsp.: (5:,3) := 5,-1,3,0", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   getmatchingparens(STR) VAL    - Gibt die Position der passenden schließenden Klammer in STR zurück. Wenn das erste Zeichen keine Klammer ist, wird die passende Klammer zur als erstes gefundenen bestimmt", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   getopt(STRING,POS)     STR    - Gibt den Wert eines Parameters in der Zeichenkette STRING, dessen Wert an der Stelle POS beginnt (z.B. nach \"=\"), als Zeichenkette zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "distrib")
        cerr << LineBreak("|   heaviside(x)           VAL    - Heaviside-(Theta)-Funktion. 0 für x < 0, 1 sonst. Definiert auf ganz R", _option, false, 0, 36) << endl;
    ...
    if (sType == "all" || sType == "string" || sType == "logic")
        cerr << LineBreak("|   is_data(STRING)        LOG    - gibt true zurück, wenn STRING ein Datenobjekt ist, sonst false", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "logic")
        cerr << LineBreak("|   is_nan(x)              LOG    - gibt true zurück, wenn x den Wert nan hat, sonst false", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string" || sType == "logic")
        cerr << LineBreak("|   is_string(EXPR)        LOG    - gibt true zurück, wenn EXPR Zeichenketten enthält, sonst false", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "polynomial")
    ...
    if (sType == "all" || sType == "num" || sType == "logarithmic")
        cerr << LineBreak("|   ln(x)                  VAL    - natürlicher Logarithmus von x. Definiert im Bereich (0,inf)", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "logarithmic")
        cerr << LineBreak("|   log10(x)               VAL    - Logarithmus zur Basis 10 von x. Kann durch 'log(x)' abgekürzt werden. Definiert im Bereich (0,inf)", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "logarithmic")
    ...
    if (sType == "all" || sType == "num" || sType == "stats")
        cerr << LineBreak("|   num(x,y,z,...)         VAL    - Zahl der gültigen Argumente x,y,z,... Kann auch auf Zeichenketten angewendet werden", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "stats")
    ...
    if (sType == "all" || sType == "num" || sType == "angular")
        cerr << LineBreak("|   radian(alpha)          VAL    - Bogenmaßwert von Winkel alpha [Grad]. Definiert für ganz R", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "random" || sType == "distrib")
    ...
    if (sType == "all" || sType == "num")
        cerr << LineBreak("|   roof(x)                VAL    - rundet x auf die nächsthöhere Ganzzahl auf", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num")
    ...
    if (sType == "all" || sType == "num" || sType == "polynomial" || sType == "physics")
        cerr << LineBreak("|   sbessel(n,x)           VAL    - sphärische Besselfunktion der Ordnung n. Definiert auf ganz R", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num")
    ...
    if (sType == "all" || sType == "num" || sType == "polynomial" || sType == "physics")
        cerr << LineBreak("|   sneumann(n,x)          VAL    - sphärische Neumannfunktion der Ordnung n. Definiert auf R \\ {0}", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
    ...
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   strfnd(STR1,STR2,P)    VAL    - Gibt die Position des ersten Auftretens von STR1 in STR2 ab dem Zeichen P zurück. P ist ein optionaler Startwert für die erste Position und standardmäßig ist P = 1. Falls nichts gefunden wird, wird false zurückgegeben", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   string_cast(EXPR)      STR    - Wandelt EXPR ungeachtet möglicher Rechenoperationen oder Zeichenkettenausdrücke in eine Zeichenkette um (#(EXPR) würde zunächst den numerischen Wert von x auswerten)", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   strlen(STRING)         VAL    - Gibt die Anzahl an Zeichen in der Zeichenkette STRING zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   strrfnd(STR1,STR2,P)   VAL    - Gibt die Position des letzten Auftretens von STR1 in STR2 bis zum Zeichen P zurück. P ist ein optionaler Startwert für letzte Position und standardmäßig ist P = strlen(STR2). Falls nichts gefunden wird, wird false zurückgegeben", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "distrib" || sType == "stats")
        cerr << LineBreak("|   student_t(n,c)         VAL    - Berechnet den zweiseitigen Student-Faktor t für n Messungen mit dem Konfidenzintervall 0 < c < 1", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   substr(STRING,n,l)     STR    - Gibt l Zeichen ab dem n-ten Zeichen aus STRING als neue Zeichenkette zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "stats")
    ...
    if (sType == "all" || sType == "num" || sType == "trigonometric")
        cerr << LineBreak("|   tan(x)                 VAL    - Tangens von x. Definiert auf R \\ {-n*PI/2, n*PI/2}. n ist eine natürliche Zahl > 0.", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "num" || sType == "hyperbolic")
    ...
    if (sType == "all" || sType == "time")
        cerr << LineBreak("|   time()                 VAL    - Gibt die Zeit in Sekunden seit dem 1.1.1970 zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   to_char(x,y,z,...)     STR    - Interpretiert x,y,z,... als ASCII-Zeichenwerte und gibt sie als gemeinsame Zeichenkette zurück", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
    ...
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   to_lowercase(STRING)   STR    - Wandelt die Großbuchstaben in STRING in Kleinbuchstaben um", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   to_string(EXPR)        STR    - Wandelt EXPR, wenn es sich um keine Zeichenkette handelt, in eine solche um (#(EXPR) würde zunächst den numerischen Wert von x auswerten)", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
        cerr << LineBreak("|   to_uppercase(STRING)   STR    - Wandelt die Kleinbuchstaben in STRING in Großbuchstaben um", _option, false, 0, 36) << endl;
    if (sType == "all" || sType == "string")
...
void parser_ListLogical(const Settings& _option)
{
    ...
    cerr << toSystemCodePage("|   x & y      - Binäres UND") << endl;
    cerr << toSystemCodePage("|   x | y      - Binäres ODER") << endl;
    cerr << toSystemCodePage("|   x < y      - WAHR, wenn x kleiner y, sonst FALSCH") << endl;
    cerr << toSystemCodePage("|   x > y      - WAHR, wenn x größer y, sonst FALSCH") << endl;
...
void parser_ListCmd(const Settings& _option)
{
    ...
    cerr << LineBreak("|   continue                    - Restlichen Schleifendurchlauf überspringen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   copy         OB [-PAR]      - Kopiert (je nach Ausdruck) Einträge/Spalten/Untertabellen zwischen Datenobjekten oder Dateien.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   data         -PAR           - Optionen auf Datensätzen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   datagrid     EX -PAR        - Datengitter erzeugen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   define       EX [-set PAR]  - Ermöglicht die Definition eigener Funktionen.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   delete       OB [-PAR]      - Entfernt Einträge aus dem Cache", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   edit         OB             - Öffnet eine Datei oder ein Verzeichnis zum Bearbeiten", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   else                        - Startet die Definition, die beim gegenteiligen Fall von \"if\" bzw. \"elseif\", ausgeführt werden soll.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   elseif       EX             - Startet eine bedingte Definition, die beim gegenteiligen Fall von \"if\" bzw. \"elseif\", ausgeführt werden soll.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   endcompose                  - Beendet die Definition einer zuvor begonnenen Plotkomposition", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   endfor       [-PAR]         - Beendet die Definition einer zuvor begonnenen Zählschleife", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   explicit     CMD            - Deaktiviert alle Plugins für das direkt darauffolgende Kommando", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   fit          OB -PAR        - Funktionen über Parameter an Datensätze anpassen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   fitw         OB -PAR        - Funktionen über Parameter an Datensätze mit Fehlerwerten anpassen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   for          EX             - Startet die Definition einer Zählschleife.", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   ifndefined   EX [-set PAR]  - Prüft vor der Definition, ob die zu definierende Funktion nicht schon bereits definiert wurde.", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   move         OB -PAR        - Verschiebt (je nach Ausdruck) Einträge/ Spalten/ Untertabellen im Cache oder Dateien.", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   odesolve     EX -set PAR    - Löst die als Ausdruck übergebenen Differentialgleichungen erster Ordnung numerisch", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   plot         EX [-set PAR]  - Ermöglicht das graphische Darstellen von Daten und Funktionen.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   plot3d       EX [-set PAR]  - graphische Darstellung dreidimensionaler Datensätze und Trajektorien.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   procedure    NM             - Definiert eine NumeRe-Prozedur", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   progress     EX [-set PAR]  - Gibt einen Fortschrittsstatus aus. Nur für Scripte und Prozeduren geeignet.", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   quit         [-PAR]         - NumeRe beenden", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   random       -PAR           - Zufallszahlengenerator mit wählbaren Verteilungen", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   resample     OB -PAR        - Ermöglicht die Änderung der Samplefrequenz von Datenreihen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   retoque      OB [-PAR]      - Entfernt ungültige Werte aus Datensätzen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   return       [EX]           - Deklariert den Rückgabewert in einer Prozedur", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   set          -PAR           - Programmeinstellung tätigen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   show         OB             - Den Inhalt eines Datensatzes anzeigen", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   smooth       OB -PAR        - Verringert das Rauschen auf Datenreihen gemäß einer gewählten Ordnung der Linearisierung.", _option, false, 0, 34) << endl;
    ...
    cerr << LineBreak("|   surf         EX [-set PAR]  - Erzeugung von Oberflächen-Plots", _option, false, 0, 34) << endl;
    cerr << LineBreak("|   surf3d       EX [-set PAR]  - Erzeugung von 3D-Oberflächen-Plots", _option, false, 0, 34) << endl;
