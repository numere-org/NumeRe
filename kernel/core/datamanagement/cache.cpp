...
bool Cache::writeSingletonToCache(Indices& _idx, const string& _sCache, double* _dData)
{
    ...
    if (_idx.vI.size())
    {
        while (_idx.nI[1] == -2 && _idx.vI.back() < ::max(_idx.nI[0], getCacheLines(_nLayer, false)-1))
        {
            _idx.vI.push_back(_idx.vI.back()+1);
        }
        while (_idx.nJ[1] == -2 && _idx.vJ.back() < ::max(_idx.nJ[0], getCacheCols(_nLayer, false)-1))
        {
            _idx.vJ.push_back(_idx.vJ.back()+1);
        }
        for (size_t i = 0; i < _idx.vI.size(); i++)
    ...
    {
        if (_idx.nI[1] == -2)
            _idx.nI[1] = ::max(_idx.nI[0], getCacheLines(_nLayer, false)-1);
        if (_idx.nJ[1] == -2)
            _idx.nJ[1] = ::max(_idx.nJ[0], getCacheCols(_nLayer, false)-1);
