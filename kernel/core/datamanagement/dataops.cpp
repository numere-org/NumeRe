...
static std::string** make_stringmatrix(MemoryManager& _data, Output& _out, Settings& _option, const std::string& sCache, long long int& nLines, long long int& nCols, int& nHeadlineCount, size_t nPrecision, bool bSave)
{
    ...
    for (long long int i = 0; i < nLines; i++)
    {
        for (long long int j = 0; j < nCols; j++)
        {
            ...
            // Handle infinity
            if (mu::isinf(abs(_data.getElement(i - nHeadlineCount, j, sCache).getNum().val))
                    && _data.getElement(i - nHeadlineCount, j, sCache).getNum().val.real() > 0)
            {
                sOut[i][j] = "inf";
                continue;
            }
            ...
            if (_out.isCompact() && !bSave)
                sprintf(cBuffer, "%.*g", 4, _data.getElement(i - nHeadlineCount, j, sCache).getNum().val.real());
            else
                sprintf(cBuffer, "%.*g", (int)nPrecision, _data.getElement(i - nHeadlineCount, j, sCache).getNum().val.real());
