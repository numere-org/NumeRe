...
// This private member function searches the indicated string
// variable occurence for possible string methods and replaces
// them with the standard string function.
void StringMemory::replaceStringMethod(string& sLine, size_t nPos, size_t nLength, const string& sVarValue)
{
    ...
    else if (sMethod == "at" || sMethod == "sub" || sMethod == "splt")
    {
        ...
        replaceAccessMethods(sLine, nPos, nFinalPos, sVarValue, sMethod, sArgument);
    }
    else if (sMethod == "fnd" || sMethod == "rfnd" || sMethod == "mtch" || sMethod == "rmtch" || sMethod == "nmtch" || sMethod == "nrmtch")
    {
        // All search-oriented methods
        replaceSearchMethods(sLine, nPos, nFinalPos, sVarValue, sMethod, sArgument);
    }
}

// This private member function replaces all search-oriented methods
// in the current string variable access
void StringMemory::replaceSearchMethods(string& sLine, size_t nPos, size_t nFinalPos, const string& sVarValue, const string& sMethod, string& sArgument)
{
...
// This private member function replaces all access-oriented methods
// and the string splitter in the current string variable access
void StringMemory::replaceAccessMethods(string& sLine, size_t nPos, size_t nFinalPos, const string& sVarValue, const string& sMethod, string& sArgument)
{
    // Prepare the argument (use empty one or construct one
    // from argument and variable value)
    if (sArgument == "()")
