...
static std::vector<Unit> tokenizeUnit(StringView sUnit)
{
    ...
    static std::vector<std::string> vTokenBoundaries = splitUtf8Chars(std::string(" *^/²³"));
    ...
    while (sUnit.length())
    {
        ...
        else
        {
            ...
            if (sUnit.match("²", nOperator) || sUnit.match("³", nOperator))
            {
    ...
    for (size_t i = 0; i < vTokens.size(); i++)
    {
        ...
        else if (vTokens.size() > i+1 && vTokens[i+1] == "²")
        {
            // Handle simple exponents
...
{
    if (sUnit.starts_with("µ") && sUnit.length() > 2)
    {
        sUnit.trim_front(2);
        return 1e-6;
    }
    ...
    if (sUnit.length() > 1 && (std::isalpha(sUnit[1]) || sUnit[1] == '°'))
    {
        ...
        if (sUnit.starts_with("u")) // e.g. um instead if µm
        {
            sUnit.trim_front(1);
            return 1e-6;
        }
