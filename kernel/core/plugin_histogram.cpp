	...
	if (_data.isValid() || _data.isValidCache())			// Sind ueberhaupt Daten vorhanden?
	{
		...
		if (!bMake2DHist)
		{
			...
			if (bGrid)
			{
				...
				{
					dMin = _data.min(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow);
					dMax = _data.max(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow);
				...
				if (!isnan(dMin) && !isnan(dMax))
				{
					if (dMin > _data.max(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow)
							|| dMax < _data.min(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow))
					{
						delete _histGraph;
				...
				{
					dMinY = _data.min(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow + 1);
					dMaxY = _data.max(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow + 1);
				...
				if (!isnan(dMinY) && !isnan(dMaxY))
				{
					if (dMinY > _data.max(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow + 1)
							|| dMaxY < _data.min(sDatatable, 0, _data.getLines(sDatatable) - 1, nDataRow + 1))
					{
						delete _histGraph;
						throw SyntaxError(SyntaxError::INVALID_INTERVAL, sCmd, SyntaxError::invalid_position);
					}
				...
				{
					dMinZ = _data.min(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow + 2, nDataRowFinal-1);
					dMaxZ = _data.max(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow + 2, nDataRowFinal-1);
				...
				if (!isnan(dMinZ) && !isnan(dMaxZ))
				{
					if (dMinZ > _data.max(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow + 2, nDataRowFinal-1)
							|| dMaxZ < _data.min(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow + 2, nDataRowFinal-1))
					{
						delete _histGraph;
						throw SyntaxError(SyntaxError::INVALID_INTERVAL, sCmd, SyntaxError::invalid_position);
					}
			...
			else
			{
				if (isnan(dMin) && isnan(dMax))
				{
					dMin = _data.min(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow, nDataRowFinal-1);
					dMax = _data.max(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow, nDataRowFinal-1);
					double dIntervall = dMax - dMin;
					dMax += dIntervall / 10.0;
					dMin -= dIntervall / 10.0;
				}
				...
				if (!isnan(dMin) && !isnan(dMax))
				{
					if (dMin > _data.max(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow, nDataRowFinal-1)
							|| dMax < _data.min(sDatatable, 0, _data.getLines(sDatatable)-1, nDataRow, nDataRowFinal-1))
					{
						delete _histGraph;
						throw SyntaxError(SyntaxError::INVALID_INTERVAL, sCmd, SyntaxError::invalid_position);
					}
			...
			if (!nBin && dIntervallLength == 0.0)
			{
				if (bGrid)
				{
					if (!nMethod)
						...
						dIntervallLength = 3.49 * _data.std(sDatatable, 0, _data.getLines(sDatatable, false) - 1, nDataRow + 2, nDataRowFinal - 1) / pow((double)nMax, 1.0 / 3.0);
					else if (nMethod == 2)
						dIntervallLength = 2.0 * (_data.pct(sDatatable, 0, _data.getLines(sDatatable, false) - 1, nDataRow + 2, nDataRowFinal - 1, 0.75) - _data.pct(sDatatable, 0, _data.getLines(sDatatable, false) - 1, nDataRow + 2, nDataRowFinal - 1, 0.25)) / pow((double)nMax, 1.0 / 3.0);
				}
				else
				{
					if (!nMethod)
						...
						dIntervallLength = 3.49 * _data.std(sDatatable, 0, _data.getLines(sDatatable, false) - 1, nDataRow) / pow((double)nMax / (double)(nDataRowFinal - nDataRow), 1.0 / 3.0);
					else if (nMethod == 2)
						dIntervallLength = 2.0 * (_data.pct(sDatatable, 0, _data.getLines(sDatatable, false) - 1, nDataRow, -1, 0.75) - _data.pct(sDatatable, 0, _data.getLines(sDatatable, false) - 1, nDataRow, -1, 0.25)) / pow((double)nMax / (double)(nDataRowFinal - nDataRow), 1.0 / 3.0);
				}
				if (_option.getbDebug())
